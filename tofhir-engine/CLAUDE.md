# tofhir-engine — core mapping/transformation engine
...
`io.tofhir.engine`. (Naming: `tofhir` ≡ Ignifyr — see the root [CLAUDE.md](../CLAUDE.md).)

## Entry points
- `Boot` ([src/main/scala/io/tofhir/engine/Boot.scala](src/main/scala/io/tofhir/engine/Boot.scala))
...
- `ToFhirEngine` — central orchestrator wiring config, repositories, the Spark session, and the mapping-job manager.
- CLI: `cli/CommandLineInterface` + `cli/command/*` (`Run`, `Load`, `Reload`, `Stop`, `Help`,
  `ListRunningMappings`, `ExtractRedCapSchemas`, …).

## Layout (`src/main/scala/io/tofhir/engine/`)
- `config/` — `ToFhirConfig`, `ToFhirEngineConfig` (read the `tofhir` HOCON block), `FunctionLibrariesConfig`.
...
4. Add a suite under `src/test/scala/io/tofhir/test/…` extending `ToFhirTestSpec`; put sample inputs
   in `src/test/resources`.

## Tests
- Unit: `mvn test -pl tofhir-engine` → suites in `io.tofhir.test`, extending `ToFhirTestSpec`
  (gives a `SparkSession`, the mapping/schema repositories, and a `RunningJobRegistry`).
- Integration: `io.tofhir.integrationtest` (e.g. `KafkaSourceIntegrationTest`) runs in the
...
- ⚠️ FHIRPath function libraries are validated at the `install` phase — a `scala-maven-plugin`
  launcher runs `ValidateFhirPathFunctionLibraries` over `io.tofhir.engine.mapping`. Keep every new
