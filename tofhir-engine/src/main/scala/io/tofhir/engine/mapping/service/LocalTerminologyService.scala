...
class LocalTerminologyService(settings: LocalFhirTerminologyServiceSettings)
            ...
            relatedFiles.get(cmf.name) match {
              case None =>
                throw FhirMappingException(
                  s"Missing toFHIR ConceptMap CSV file '${cmf.name}' within configured folder '${settings.folderPath}'!"
                )
      ...
      .map(cs =>
        cs.codeSystem -> parseCodeSystemGivenInCsv(
          relatedFiles.get(cs.name) match {
            case None =>
              throw FhirMappingException(
                s"Missing tofhir CodeSystem CSV file '${cs.name}' within configured folder '${settings.folderPath}'!"
              )
            case Some(f) => f.getAbsolutePath
          }
        )
  ...
  private def parseConceptMapGivenInCsv(
    ...
    try {
      ...
      case t: Throwable =>
        throw FhirMappingException(
          s"Invalid tofhir concept map CSV file $filePath! Columns ${Set(ConceptMapFileColumns.SOURCE_SYSTEM, ConceptMapFileColumns.SOURCE_CODE, ConceptMapFileColumns.TARGET_SYSTEM, ConceptMapFileColumns.TARGET_CODE).mkString(",")} are mandatory for concept map!",
          t
        )
    }
  }
  ...
  private def parseCodeSystemGivenInCsv(filePath: String): Map[String, (String, Map[String, String])] = {
    try {
      ...
      case t: Throwable =>
        throw FhirMappingException(
          s"Invalid tofhir CodeSystem CSV file $filePath! Columns ${Set(CodeSystemFileColumns.CODE, CodeSystemFileColumns.DISPLAY).mkString(",")} are mandatory for code system definitions!",
          t
        )
    }
  }
