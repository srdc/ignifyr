 ...
 */
class FhirServerDataSourceReader(spark: SparkSession)
    extends BaseDataSourceReader[FhirServerSource, FhirServerSourceSettings] {
   ...
   */
  override def read(
      mappingSourceBinding: FhirServerSource,
  ...
  private def extractAuthenticationOptions(sourceSettings: FhirServerSourceSettings): Map[String, String] = {
    sourceSettings.securitySettings
      .map {
        case BearerTokenAuthorizationSettings(
              clientId,
              clientSecret,
              requiredScopes,
