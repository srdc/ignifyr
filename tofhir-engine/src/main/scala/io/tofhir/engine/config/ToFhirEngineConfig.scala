...
class ToFhirEngineConfig(toFhirEngineConfig: Config) {
  ...
  lazy val mappingRepositoryFolderPath: String =
    Try(toFhirEngineConfig.getString("mappings.repository.folder-path")).getOrElse("mappings")

  /** Path to the folder where the mapping context files are kept. */
  lazy val mappingContextRepositoryFolderPath: String =
    Try(toFhirEngineConfig.getString("mappings.contexts.repository.folder-path")).getOrElse("mapping-contexts")

  /** Path to the folder where the schema definitions are kept. */
  lazy val schemaRepositoryFolderPath: String =
    Try(toFhirEngineConfig.getString("mappings.schemas.repository.folder-path")).getOrElse("schemas")
  ...
  lazy val schemaRepositoryFhirVersion: String =
    Try(toFhirEngineConfig.getString("mappings.schemas.fhir-version")).getOrElse("4.0.1")

  /** Path to the folder where the job definitions are kept. */
  lazy val jobRepositoryFolderPath: String =
    Try(toFhirEngineConfig.getString("mapping-jobs.repository.folder-path")).getOrElse("mapping-jobs")

  /** Path to the folder where the terminology system definitions are kept. */
  lazy val terminologySystemFolderPath: String =
    Try(toFhirEngineConfig.getString("terminology-systems.folder-path")).getOrElse("terminology-systems")

  /** Timeout for a single mapping */
  lazy val mappingTimeout: Duration = Try(toFhirEngineConfig.getDuration("mappings.timeout").toScala).toOption
  ...
  lazy val initialMappingJobFilePath: Option[String] = Try(
    toFhirEngineConfig.getString("mapping-jobs.initial-job-file-path")
  ).toOption
  ...
  lazy val partitionsForMappingJobs: Option[Int] = Try(
    toFhirEngineConfig.getInt("mapping-jobs.numOfPartitions")
  ).toOption
  ...
  lazy val maxChunkSizeForMappingJobs: Option[Long] = Try(
    toFhirEngineConfig.getLong("mapping-jobs.maxChunkSize")
  ).toOption
  ...
  lazy val fhirWriterBatchGroupSize: Int =
    Try(toFhirEngineConfig.getInt("fhir-server-writer.batch-group-size")).getOrElse(10)
  ...
  lazy val erroneousRecordsFolder: String = FileUtils
    .getPath(
      Try(toFhirEngineConfig.getString("archiving.erroneous-records-folder")).getOrElse("erroneous-records-folder")
    )
    .toString

  /** Folder path where the archive of the processed source data is stored. */
  lazy val archiveFolder: String = FileUtils
    .getPath(Try(toFhirEngineConfig.getString("archiving.archive-folder")).getOrElse("archive-folder"))
    .toString

  /** Period (in milliseconds) to run archiving task for file streaming jobs */
  lazy val streamArchivingFrequency: Int =
    Try(toFhirEngineConfig.getInt("archiving.stream-archiving-frequency")).toOption.getOrElse(5000)

  /** Configuration of external function libraries */
  lazy val functionLibrariesConfig: Option[FunctionLibrariesConfig] = Try(
    new FunctionLibrariesConfig(toFhirEngineConfig.getConfig("functionLibraries"))
  ).toOption
}
