...
import io.tofhir.engine.util.mappinggenerator.{
  GeneratorDBAdapter,
  OmopConcept,
  OmopConceptRelationship,
  TerminologyMappingGenerator,
...
class TerminologyMappingGeneratorTest extends AnyFlatSpec with Matchers with PrivateMethodTester {
  ...
  when(mockDbAdapter.getConcepts("SNOMED", "Some domain")).thenReturn(
    Set(
  ...
  when(mockDbAdapter.getConcepts("ICD10", "Some domain")).thenReturn(
    Set(
      OmopConcept(4, "ICD10", "ICD10-1", "ICD10-1")
    )
  )
  when(mockDbAdapter.getOmopConceptRelationships(Set("Mapped from"))).thenReturn(
    Set(
  ...
  "TerminologySystemMappingGenerator" should "construct concepts and relationships" in {
    val populateConceptsAndRelationships =
      PrivateMethod[Set[TerminologySystemMapping]](Symbol("populateConceptsAndRelationships"))
