...
class FhirDefinitionsEndpointTest extends BaseEndpointTest with OnFhirTestContainer {
  ...
  val patientBundleJson: String =
    Source.fromInputStream(getClass.getResourceAsStream("/fhir-resources/patient-bundle.json")).mkString
  // Valid condition resource
  val conditionResourceJson: String =
    Source.fromInputStream(getClass.getResourceAsStream("/fhir-resources/condition-resource.json")).mkString
  // Condition resource with missing subject
  val invalidConditionResourceJson: String =
    Source.fromInputStream(getClass.getResourceAsStream("/fhir-resources/invalid-condition-resource.json")).mkString
  ...
  "Fhir definitions endpoint" should {
    ...
    "validate FHIR resources" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_VALIDATE}",
        HttpEntity(ContentTypes.`application/json`, conditionResourceJson)
      ) ~> route ~> check {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_VALIDATE}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIRVALIDATIONURL}=test-url",
        HttpEntity(ContentTypes.`application/json`, conditionResourceJson)
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
      }
      ...
      Post(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_VALIDATE}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIRVALIDATIONURL}=${this.onFhirClient.getBaseUrl()}/Condition/$$validate",
        HttpEntity(ContentTypes.`application/json`, conditionResourceJson)
      ) ~> route ~> check {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_VALIDATE}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIRVALIDATIONURL}=${this.onFhirClient.getBaseUrl()}/Condition/$$validate",
        HttpEntity(ContentTypes.`application/json`, invalidConditionResourceJson)
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        (errorIssue \ "diagnostics") should be(
          JString(
            "[Validating against 'http://hl7.org/fhir/StructureDefinition/Condition'] => Element 'subject' with data type(s) 'Reference' is required , but does not exist!"
          )
        )
      ...
      Post(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_VALIDATE}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIRVALIDATIONURL}=${this.onFhirClient.getBaseUrl()}",
        HttpEntity(ContentTypes.`application/json`, patientBundleJson)
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        (secondEntryFirstIssue \ "diagnostics").asInstanceOf[JString].s.split(" => ").last should be(
          "Invalid value '1' for FHIR primitive type 'boolean'!"
        )
        ...
        (thirdEntryFirstIssue \ "diagnostics").asInstanceOf[JString].s.split(" => ").last should be(
          "Invalid value 'test' for FHIR primitive type 'date'!"
        )
    ...
    "retrieve base FHIR resources" in {
      ...
      Get(s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_BASE_PROFILES}") ~> route ~> check {
        ...
        responseAs[String] should include(
          "Detail: base-profiles path cannot be invoked without the query parameter 'fhirVersion'."
        )
      }
       ...
       */
      Get(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_BASE_PROFILES}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIR_VERSION}=${MajorFhirVersion.R4}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
       ...
       */
      Get(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_BASE_PROFILES}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIR_VERSION}=R6"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
        responseAs[String] should include(
          "Detail: fhirVersion on base-profiles cannot take the value: R6. Possible values are: R4 and R5"
        )
      }
       ...
       */
      Get(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_BASE_PROFILES}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIR_VERSION}=${MajorFhirVersion.R4}&${FhirDefinitionsEndpoint.QUERY_PARAM_PROFILE}=http://hl7.org/fhir/StructureDefinition/Patient"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
       ...
       */
      Get(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_BASE_PROFILES}?${FhirDefinitionsEndpoint.QUERY_PARAM_FHIR_VERSION}=${MajorFhirVersion.R4}&${FhirDefinitionsEndpoint.QUERY_PARAM_PROFILE}=http://hl7.org/fhir/StructureDefinition/Patient,http://hl7.org/fhir/StructureDefinition/Condition"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        val responseBody = JsonMethods.parse(responseAs[String]).extract[Seq[SchemaDefinition]]
    ...
    "retrieve simplified element definitions of a FHIR profile" in {
      Get(
        s"/${webServerConfig.baseUri}/${FhirDefinitionsEndpoint.SEGMENT_FHIR_DEFINITIONS}?$QUERY_PARAM_Q=${DefinitionsQuery.ELEMENTS}&$QUERY_PARAM_PROFILE=http://hl7.org/fhir/StructureDefinition/Patient"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
