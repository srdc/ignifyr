...
class ExtractRedCapSchemas extends Command {
  ...
  override def execute(args: Seq[String], context: CommandExecutionContext): CommandExecutionContext = {
    if (args.isEmpty) {
      println(
        "extract-redcap-schemas command requires the path of RedCap data dictionary to extract from and definition root url."
      )
      context
      ...
      try {
        ...
        fhirResources.foreach(fhirResource => {
          ...
          val file = FileUtils
            .getPath(IgnifyrConfig.engineConfig.schemaRepositoryFolderPath, resourceType + ".StructureDefinition.json")
            .toFile
          ...
          try fw.write(writePretty(fhirResource))
          finally fw.close()
