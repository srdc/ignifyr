package io.ignifyr.server.repository.mappingContext
...
import io.ignifyr.server.common.model.{AlreadyExists, ResourceNotFound}
import io.ignifyr.server.model._
import io.ignifyr.server.model.csv.CsvHeader
import io.ignifyr.server.repository.project.IProjectRepository
import io.ignifyr.server.util.CsvUtil
 ...
 */
class MappingContextFolderRepository(mappingContextRepositoryFolderPath: String, projectRepository: IProjectRepository)
    extends IMappingContextRepository {
  ...
  override def createMappingContext(projectId: String, mappingContextId: String): Future[String] = {
    if (mappingContextExists(projectId, mappingContextId)) {
      throw AlreadyExists(
        ...
        s"A mapping context definition with id $mappingContextId already exists in the mapping context repository at ${FileUtils.getPath(mappingContextRepositoryFolderPath).toAbsolutePath.toString}"
      )
    }
    ...
    getFileForMappingContext(projectId, mappingContextId).flatMap(newFile => {
      ...
      projectRepository.addMappingContext(projectId, mappingContextId) map { _ =>
        ...
        mappingContextDefinitions(projectId) =
          mappingContextDefinitions.getOrElseUpdate(projectId, Seq.empty) :+ mappingContextId
        mappingContextId
      }
    })
  }
  ...
  override def deleteMappingContext(projectId: String, mappingContextId: String): Future[Unit] = {
    if (!mappingContextExists(projectId, mappingContextId)) {
      throw ResourceNotFound(
        "Mapping context does not exists.",
        s"A mapping context with id $mappingContextId does not exists in the mapping context repository at ${FileUtils.getPath(mappingContextRepositoryFolderPath).toAbsolutePath.toString}"
      )
    }
  ...
  override def deleteAllMappingContexts(projectId: String): Unit = {
    Future {
      ...
      org.apache.commons.io.FileUtils
        .deleteDirectory(FileUtils.getPath(mappingContextRepositoryFolderPath, projectId).toFile)
   ...
   * @return
   */
  override def saveMappingContextContent(
      projectId: String,
   ...
   * @return
   */
  override def getMappingContextContent(
      projectId: String,
      mappingContextId: String,
      pageNumber: Int,
      pageSize: Int
   ...
   * @return
   */
  override def uploadMappingContext(
      projectId: String,
      mappingContextId: String,
      content: Source[ByteString, Any]
  ): Future[Unit] = {
  ...
  private def getFileForMappingContext(projectId: String, fhirMappingContextId: String): Future[File] = {
    ...
    projectFuture.map(project => {
      val file: File =
        FileUtils.getPath(mappingContextRepositoryFolderPath, project.get.id, fhirMappingContextId).toFile
  ...
  private def initMap(mappingContextRepositoryFolderPath: String): Unit = {
    ...
    directories.foreach { projectDirectory =>
      val files =
        IOUtil.getFilesFromFolder(projectDirectory, recursively = true, ignoreHidden = true, withExtension = None)
