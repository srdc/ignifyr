...
import io.ignifyr.engine.{Execution, IgnifyrEngine}
import io.ignifyr.server.common.model.{BadRequest, ResourceNotFound}
import io.ignifyr.server.model.{ExecuteJobTask, TestResourceCreationRequest}
...
class ExecutionService(
  ...
  val ignifyrEngine = new IgnifyrEngine(Some(mappingRepository), Some(schemaRepository))
  ...
  def runJob(
    ...
    jobRepository.getJob(projectId, jobId) map {
      ...
      case Some(mj) =>
        ...
        val JobExecutionMap = ignifyrEngine.runningJobRegistry.getRunningExecutions()
      ...
      case Some(mj) =>
          ...
          testResourceCreationRequest.fhirMappingTask.mapping match {
            ...
            case _ =>
              ...
              val pathToMappingFile: File = FileUtils
                .getPath(
                  IgnifyrConfig.engineConfig.mappingRepositoryFolderPath,
                  projectId,
        ...
        fhirMappingJobManager
          ...
          .map { resultingDataFrame =>
            ...
            import ignifyrEngine.sparkSession.implicits._
  ...
  def getExecutions(projectId: String, jobId: String): Future[Seq[JValue]] = {
    ...
    jobRepository.getJob(projectId, jobId).flatMap {
      case Some(_) =>
        ...
        val runningExecutionsJson: Seq[JValue] = ignifyrEngine.runningJobRegistry
  ...
  def stopJobExecution(jobId: String, executionId: String): Future[Unit] = {
    Future {
      if (ignifyrEngine.runningJobRegistry.executionExists(jobId, executionId, None)) {
        ignifyrEngine.runningJobRegistry.stopJobExecution(jobId, executionId)
  ...
  def stopJobExecutions(jobId: String): Future[Unit] = {
    Future {
      ignifyrEngine.runningJobRegistry.stopJobExecutions(jobId)
    }
  }
  ...
  def stopMappingExecution(jobId: String, executionId: String, mappingTaskName: String): Future[Unit] = {
    Future {
      if (ignifyrEngine.runningJobRegistry.executionExists(jobId, executionId, Some(mappingTaskName))) {
        ignifyrEngine.runningJobRegistry.stopMappingExecution(jobId, executionId, mappingTaskName)
        logger.debug(
  ...
  def isJobRunning(jobId: String): Future[Boolean] = {
    Future {
      ignifyrEngine.runningJobRegistry.isJobRunning(jobId)
    }
  }
