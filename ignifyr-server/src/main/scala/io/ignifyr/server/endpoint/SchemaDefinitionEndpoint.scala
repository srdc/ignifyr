...
import io.ignifyr.server.endpoint.SchemaDefinitionEndpoint.{
  SEGMENT_IMPORT,
  SEGMENT_IMPORT_ZIP,
  SEGMENT_INFER,
  SEGMENT_REDCAP,
...
import io.ignifyr.server.service.SchemaDefinitionService
...
class SchemaDefinitionEndpoint(schemaRepository: ISchemaRepository, mappingRepository: IMappingRepository)
  ...
  private def getAllSchemas(request: IgnifyrRestCall): Route = {
    get {
...
object SchemaFormats {
   ...
   * by a {@link io.ignifyr.common.model.SimpleStructureDefinition}. This format provides a simplified structure for the schema's elements.
   */
