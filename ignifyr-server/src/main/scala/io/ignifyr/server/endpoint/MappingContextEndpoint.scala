...
import io.ignifyr.server.endpoint.MappingContextEndpoint.{
  ATTACHMENT,
  SEGMENT_CONTENT,
  SEGMENT_CONTEXTS,
  SEGMENT_FILE,
...
import io.ignifyr.server.service.MappingContextService
...
class MappingContextEndpoint(mappingContextRepository: IMappingContextRepository) extends LazyLogging {
   ...
   * @return
   */
  private def createMappingContext(request: IgnifyrRestCall, projectId: String): Route = {
