...
object IgnifyrHttpServer extends LazyLogging {
  ...
  def start(route: Route, webServerConfig: WebServerConfig)(implicit actorSystem: ActorSystem): Unit = {
    ...
    val serverBindingFuture = Http()
      ...
      .map(serverBinding => {
        ...
        serverBinding.whenTerminated onComplete {
          case Success(_) =>
            logger.info("Closing Ignifyr HTTP server...")
            actorSystem.terminate()
            logger.info("Ignifyr HTTP server gracefully terminated.")
          case Failure(exception) =>
            logger.error("Problem while gracefully terminating Ignifyr HTTP server!", exception)
        }
        serverBinding
      })
    ...
    try {
      ...
      logger.info(s"Ignifyr server ready at ${webServerConfig.serverHost}:${webServerConfig.serverPort}")
    } catch {
      case e: Exception =>
