...
class IgnifyrRejectionHandlerTest extends BaseEndpointTest {
  val project1: Project = Project(name = "example", description = Some("example project"))

  "IgnifyrRejectionHandler" should {
    ...
    "create an HTTP response with unsupported media type status for invalid content type" in {
      ...
      Post(
        ...
        akka.http.scaladsl.model.HttpEntity.apply(writePretty(project1))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.UnsupportedMediaType
        val response = responseAs[String]
        response should include("Type: https://ignifyr.io/errors/UnsupportedMediaType")
      }
    }
    ...
    "create an HTTP response with bad request status for content not complying with the data model" in {
      ...
      val projectWithoutUrl =
        Map("url" -> "http://example3.com/example-project", "description" -> Some("example project"))
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}",
        akka.http.scaladsl.model.HttpEntity.apply(ContentTypes.`application/json`, writePretty(projectWithoutUrl))
      ) ~> route ~> check {
    ...
    "create an HTTP response with method not allowed status for requests initiated with an unsupported method" in {
      ...
      Put(
        ...
        val response = responseAs[String]
        response should include("Type: https://ignifyr.io/errors/MethodForbidden")
      }
    }
    ...
    "create an HTTP response with not found status for wrong URL" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/invalidURL",
        akka.http.scaladsl.model.HttpEntity.apply(ContentTypes.`application/json`, writePretty(project1))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.NotFound
        val response = responseAs[String]
        response should include("Type: https://ignifyr.io/errors/ResourceNotFound")
