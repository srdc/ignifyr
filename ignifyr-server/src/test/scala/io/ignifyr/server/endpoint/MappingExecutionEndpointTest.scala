...
class MappingExecutionEndpointTest extends BaseEndpointTest with OnFhirTestContainer {
  ...
  "The service" should {
    "run a job including a mapping" in {
      ...
      Post(
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.jobRepositoryFolderPath, projectId, s"${batchJob.id}${FileExtensions.JSON}")
          .toFile should exist
      }
    ...
    "execute a mapping that is included in the mapping task" in {
      ...
      Post(
        ...
        // check job folder is created
        FileUtils
          .getPath(ignifyrEngineConfig.jobRepositoryFolderPath, projectId, s"${job2.id}${FileExtensions.JSON}")
          .toFile should exist
      }
    ...
    "run a streaming job including a mapping" in {
      ...
      Post(
        ...
        // check job folder is created
        FileUtils
          .getPath(ignifyrEngineConfig.jobRepositoryFolderPath, projectId, s"${streamingJob.id}${FileExtensions.JSON}")
          .toFile should exist
      }
  ...
  private def createSchemaAndVerify(schemaDefinitionResourceFile: String, expectedSchemaCount: Int): Unit = {
    ...
    Post(
      ...
      FileUtils
        .getPath(
          ignifyrEngineConfig.schemaRepositoryFolderPath,
          projectId,
  ...
  def createContextMapAndVerify(conceptMapResourceFile: String, mappingContextId: String): Unit = {
    ...
    Post(
      ...
      FileUtils
        .getPath(ignifyrEngineConfig.mappingContextRepositoryFolderPath, projectId, mappingContextId)
        .toFile should exist
    }
  ...
  override def afterAll(): Unit = {
    ...
    org.apache.commons.io.FileUtils.deleteDirectory(Paths.get(IgnifyrConfig.sparkCheckpointDirectory).toFile)
