...
import io.ignifyr.server.model.{Project, ProjectEditableFields}
    ...
    "delete a project" in {
      ...
      Post(
        ...
        val firstProject = TestUtil
          .getProjectJsonFile(ignifyrEngineConfig)
          .arr
        ...
        FileUtils.getPath(ignifyrEngineConfig.schemaRepositoryFolderPath, project1.id).toFile should exist
      }
      ...
      Delete(s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/${project1.id}") ~> route ~> check {
        ...
        FileUtils.getPath(ignifyrEngineConfig.schemaRepositoryFolderPath, project1.id).toFile shouldNot exist
      }
