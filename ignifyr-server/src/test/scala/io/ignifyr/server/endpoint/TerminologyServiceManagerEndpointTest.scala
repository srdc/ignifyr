...
import io.ignifyr.server.model.TerminologySystem
import io.ignifyr.server.model.TerminologySystem.{TerminologyCodeSystem, TerminologyConceptMap}
import io.ignifyr.server.repository.terminology.TerminologySystemFolderRepository.getTerminologySystemsJsonPath
...
class TerminologyServiceManagerEndpointTest extends BaseEndpointTest {
  var conceptMap1: TerminologyConceptMap =
    ConceptMapFile(name = "testCM", conceptMapUrl = "", sourceValueSetUrl = "", targetValueSetUrl = "")
  var conceptMap2: TerminologyConceptMap =
    ConceptMapFile(name = "testCM2", conceptMapUrl = "", sourceValueSetUrl = "", targetValueSetUrl = "")
  ...
  var terminologySystem1: TerminologySystem = TerminologySystem(
    name = "testTerminology1",
    description = "example terminology 1",
    codeSystems = Seq.empty,
    conceptMaps = Seq.empty
  )

  val terminologySystem2: TerminologySystem = TerminologySystem(
    name = "testTerminology2",
    description = "example terminology 2",
    codeSystems = Seq.empty,
    conceptMaps = Seq.empty
  )
  ...
  val terminologyServiceSettings: TerminologyServiceSettings = LocalFhirTerminologyServiceSettings(
    ...
    codeSystemFiles = Seq.empty,
    conceptMapFiles = Seq.empty
  )
  val jobTest: FhirMappingJob = FhirMappingJob(
    name = Some("mappingJobTest"),
  ...
  "The terminology service" should {
    "create a terminology system" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/terminologies",
        HttpEntity(ContentTypes.`application/json`, writePretty(terminologySystem1))
      ) ~> route ~> check {
        ...
        FileUtils.getPath(ignifyrEngineConfig.terminologySystemFolderPath, terminologySystem.id).toFile should exist
        // validate that terminologies metadata file is updated
        val terminologySystems = FileOperations.readJsonContent[TerminologySystem](
          FileUtils.getPath(getTerminologySystemsJsonPath(ignifyrEngineConfig.terminologySystemFolderPath)).toFile
        )
        terminologySystems.length shouldEqual 1
      }
      // create the second terminology
      Post(
        s"/${webServerConfig.baseUri}/terminologies",
        HttpEntity(ContentTypes.`application/json`, writePretty(terminologySystem2))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        // validate that terminologies metadata file is updated
        val terminologySystems = FileOperations.readJsonContent[TerminologySystem](
        ...
        terminologySystems.length shouldEqual 2
      }
    }
    ...
    "get all terminology systems" in {
      ...
      Get(s"/${webServerConfig.baseUri}/terminologies") ~> route ~> check {
        ...
        val terminologySystems: Seq[TerminologySystem] =
          JsonMethods.parse(responseAs[String]).extract[Seq[TerminologySystem]]
        terminologySystems.length shouldEqual 2
      }
    }
    ...
    "put a terminology system" in {
      ...
      Put(
        s"/${webServerConfig.baseUri}/terminologies/${terminologySystem1.id}",
        HttpEntity(ContentTypes.`application/json`, writePretty(terminologySystem1.copy(name = "nameUpdated1")))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        terminologySystems
          .find(_.id == terminologySystem.id)
          .get
          .name shouldEqual "nameUpdated1"
        terminologySystem1 = terminologySystem
      }
    }
    ...
    "delete a terminology system" in {
      ...
      Delete(s"/${webServerConfig.baseUri}/terminologies/${terminologySystem2.id}") ~> route ~> check {
        ...
        FileUtils.getPath(ignifyrEngineConfig.terminologySystemFolderPath, terminologySystem2.id).toFile shouldNot exist
  ...
  "The terminology concept map service" should {
    "create a job and reference to a terminology system" in {
      Post(
        ...
        HttpEntity(ContentTypes.`application/json`, writePretty(jobTest))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.jobRepositoryFolderPath, projectId, s"${jobTest.id}${FileExtensions.JSON}")
          .toFile should exist
      }
    }

    // add a concept map to the terminology system object
    var updatedTerminology: TerminologySystem =
      terminologySystem1.copy(conceptMaps = terminologySystem1.conceptMaps :+ conceptMap1)

    "create a concept map within a terminology system" in {
      // create a concept map
      Put(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}",
        HttpEntity(ContentTypes.`application/json`, writePretty(updatedTerminology))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        FileUtils
          .getPath(
            ...
            terminologySystem1.id,
            updatedTerminology.conceptMaps.last.id
          )
          .toFile should exist
      }
      // add a concept map to the terminology system object
    ...
    "get concept maps within a terminology system" in {
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${ConceptMapEndpoint.SEGMENT_CONCEPT_MAPS}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate that it returns one concept maps
        val conceptMaps: Seq[TerminologyConceptMap] =
          JsonMethods.parse(responseAs[String]).extract[Seq[TerminologyConceptMap]]
    ...
    "get a concept map within a terminology system" in {
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${ConceptMapEndpoint.SEGMENT_CONCEPT_MAPS}/${conceptMap1.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${ConceptMapEndpoint.SEGMENT_CONCEPT_MAPS}/Non-existent"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.NotFound
      }
    }
    ...
    "get the job in a project to verify terminology system is do not updated with new conceptMaps" in {
      // get the job
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/${jobTest.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        job.terminologyServiceSettings.get
          .asInstanceOf[LocalFhirTerminologyServiceSettings]
          .conceptMapFiles
    ...
    "put a concept map within a terminology system" in {
      ...
      updatedTerminology = updatedTerminology.copy(conceptMaps =
        updatedTerminology.conceptMaps.map(conceptMap => conceptMap.copy(name = conceptMap.name + "Updated"))
      )
    ...
    "delete a concept map within a terminology system" in {
      ...
      Put(
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.terminologySystemFolderPath, terminologySystem1.id, conceptMap2.id)
    ...
    "upload a csv content as a concept map within a terminology system" in {
      ...
      val file: File =
        FileOperations.getFileIfExists(getClass.getResource("/test-terminology-service/sample-concept-map.csv").getPath)
      ...
      Post(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${ConceptMapEndpoint.SEGMENT_CONCEPT_MAPS}/${conceptMap1.id}/${TerminologyServiceManagerEndpoint.SEGMENT_FILE}",
        formData.toEntity()
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
  ...
  "The terminology code system service" should {
    ...
    var updatedTerminology: TerminologySystem =
      terminologySystem1.copy(codeSystems = terminologySystem1.codeSystems :+ codeSystem1)
      ...
      Put(
        ...
        FileUtils
          .getPath(
            ignifyrEngineConfig.terminologySystemFolderPath,
            terminologySystem1.id,
            updatedTerminology.codeSystems.last.id
          )
          .toFile should exist
      }
      // add a code system to the terminology system object
    ...
    "get code systems within a terminology system" in {
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${CodeSystemEndpoint.SEGMENT_CODE_SYSTEMS}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate that it returns one code systems
        val codeSystems: Seq[TerminologyCodeSystem] =
          JsonMethods.parse(responseAs[String]).extract[Seq[TerminologyCodeSystem]]
    ...
    "get a code system within a terminology system" in {
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${CodeSystemEndpoint.SEGMENT_CODE_SYSTEMS}/${codeSystem1.id}"
      ...
      Get(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${CodeSystemEndpoint.SEGMENT_CODE_SYSTEMS}/Non-existent"
      ).addHeader(RawHeader("Content-Type", "application/json")) ~> route ~> check {
        status shouldEqual StatusCodes.NotFound
      }
    }
    ...
    "put a code system within a terminology system" in {
      ...
      updatedTerminology = updatedTerminology.copy(codeSystems =
        updatedTerminology.codeSystems.map(codeSystem => codeSystem.copy(name = codeSystem.name + "Updated"))
      )
    ...
    "delete a code system within a terminology system" in {
      ...
      Put(
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.terminologySystemFolderPath, terminologySystem1.id, codeSystem2.id)
          .toFile shouldNot exist
      }
    }
    ...
    "upload a csv content as a code system within a terminology system" in {
      // get file from resources
      val file: File =
        FileOperations.getFileIfExists(getClass.getResource("/test-terminology-service/sample-code-system.csv").getPath)
      val fileData = Multipart.FormData.BodyPart.fromPath("attachment", ContentTypes.`text/plain(UTF-8)`, file.toPath)
      val formData = Multipart.FormData(fileData)
      // save a csv file as a concept map within a terminology system
      Post(
        s"/${webServerConfig.baseUri}/${TerminologyServiceManagerEndpoint.SEGMENT_TERMINOLOGY}/${terminologySystem1.id}/${CodeSystemEndpoint.SEGMENT_CODE_SYSTEMS}/${codeSystem1.id}/${TerminologyServiceManagerEndpoint.SEGMENT_FILE}",
        formData.toEntity()
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        responseAs[String] shouldEqual "OK"
