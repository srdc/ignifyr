...
import io.ignifyr.server.util.TestUtil
...
class JobEndpointTest extends BaseEndpointTest {
  ...
  val kafkaSourceSettings: KafkaSourceSettings = KafkaSourceSettings(
    name = "kafka-source",
    sourceUri = "http://example.com/kafka",
    bootstrapServers = "http://some-kafka-server:9092"
  )
  val mappingJobSourceSettings: Map[String, MappingJobSourceSettings] =
    Map("source1" -> kafkaSourceSettings)
  val kafkaMappingTask: Seq[FhirMappingTask] = Seq(
    FhirMappingTask(
      "mappingTest1",
      "mappingRef1",
      Map(
        "sourceBinding1" -> KafkaSource(
          topicName = "topic",
          sourceRef = Some("source1"),
          options = Map("startingOffsets" -> "latest")
        )
      )
    )
  )
  val kafkaSourceJob: FhirMappingJob = FhirMappingJob(
    name = Some("mappingJob2"),
    sourceSettings = mappingJobSourceSettings,
    sinkSettings = sinkSettings,
    mappings = kafkaMappingTask,
    dataProcessingSettings = DataProcessingSettings()
  )
  // a malformed job with a source reference to a missing data source in the mapping tasks, to be rejected
  val malformedMappingTasks: Seq[FhirMappingTask] = Seq(
    FhirMappingTask(
      "mappingTest2",
      "mappingRef1",
      Map("sourceBinding1" -> SqlSource(tableName = Some("table"), sourceRef = Some("source2")))
    )
  )
  val mappingTaskMalformedJob: FhirMappingJob = FhirMappingJob(
    name = Some("malformedJob1"),
    sourceSettings = mappingJobSourceSettings,
    sinkSettings = sinkSettings,
    mappings = malformedMappingTasks,
    dataProcessingSettings = DataProcessingSettings()
  )
  // a malformed job which is a scheduling job and has a stream file system data source, to be rejected
  val streamFileSystemSourceSettings: FileSystemSourceSettings = FileSystemSourceSettings(
    name = "file-system-source",
    sourceUri = "http://example.co/filesystem",
    dataFolderPath = "test/data",
    asStream = true
  )
  val streamAndSchedulingMalformedJob: FhirMappingJob = FhirMappingJob(
    name = Some("malformedJob2"),
    schedulingSettings = Some(SchedulingSettings(cronExpression = "* * * * *")),
    sourceSettings = Map(
      "source1" -> streamFileSystemSourceSettings.copy(asStream = false),
      "source2" -> streamFileSystemSourceSettings
    ),
    sinkSettings = sinkSettings,
    mappings = Seq.empty,
    dataProcessingSettings = DataProcessingSettings()
  ...
  "The service" should {
    ...
    "create a job within project" in {
      ...
      Post(
        ...
        HttpEntity(ContentTypes.`application/json`, writePretty(job1))
      ) ~> route ~> check {
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.jobRepositoryFolderPath, projectId, s"${job1.id}${FileExtensions.JSON}")
          .toFile should exist
      }
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}",
        HttpEntity(ContentTypes.`application/json`, writePretty(kafkaSourceJob))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        // validate that job metadata file is updated
        val projects: JArray = TestUtil.getProjectJsonFile(ignifyrEngineConfig)
        ...
        FileUtils
          .getPath(
            ignifyrEngineConfig.jobRepositoryFolderPath,
            projectId,
            s"${kafkaSourceJob.id}${FileExtensions.JSON}"
          )
          .toFile should exist
      }
    }
    ...
    "get a job in a project" in {
      // get a job
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/${job1.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      ...
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/123123"
      ) ~> route ~> check {
    ...
    "update a job in a project" in {
      ...
      Put(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/${job1.id}",
        HttpEntity(ContentTypes.`application/json`, writePretty(job1.copy(name = Some("updatedJob"))))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      ...
      Put(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/123123",
        HttpEntity(ContentTypes.`application/json`, writePretty(job1.copy(id = "123123")))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.NotFound
      }
    }
    ...
    "get a job with kafka source type in a project" in {
      // get a job
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}/${kafkaSourceJob.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
    ...
    "prevent creation of jobs with malformed content" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}",
        HttpEntity(ContentTypes.`application/json`, writePretty(mappingTaskMalformedJob))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
        // validate error message
        val response = responseAs[String]
        response should include(
          "Detail: The data source referenced by source name 'source2' in the mapping task 'mappingTest2' is not found in the source settings of the job."
        )
        ...
        FileUtils
          .getPath(
            ignifyrEngineConfig.jobRepositoryFolderPath,
            projectId,
            s"${mappingTaskMalformedJob.id}${FileExtensions.JSON}"
          )
          .toFile shouldNot exist
      }
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}",
        HttpEntity(ContentTypes.`application/json`, writePretty(streamAndSchedulingMalformedJob))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
        // validate error message
        val response = responseAs[String]
  ...
  "prevent creation of a job with two mappingTask share same name" in {
    ...
    val jobWithSameMappingTaskName = job1.copy(
      id = "jobWithSameMappingTaskName",
      name = Option("jobWithSameMappingTaskName"),
      mappings = Seq(mappingTask, mappingTask2)
    )
    // try to create a job with two mappingTask share same name
    Post(
      s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${JobEndpoint.SEGMENT_JOB}",
      HttpEntity(ContentTypes.`application/json`, writePretty(jobWithSameMappingTaskName))
    ) ~> route ~> check {
      status shouldEqual StatusCodes.BadRequest
      // validate error message
      val response = responseAs[String]
      response should include(
        "Detail: Duplicate MappingTask name(s) found: testname1. Each MappingTask must have a unique name."
      )
      // validate that job metadata file is still the same
      val projects: JArray = TestUtil.getProjectJsonFile(ignifyrEngineConfig)
      (projects.arr.find(p => (p \ "id").extract[String] == projectId).get \ "mappingJobs")
      ...
      FileUtils
        .getPath(
          ignifyrEngineConfig.jobRepositoryFolderPath,
          projectId,
          s"${jobWithSameMappingTaskName.id}${FileExtensions.JSON}"
        )
        .toFile shouldNot exist
    }
  }
