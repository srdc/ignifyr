...
import io.ignifyr.server.endpoint.{MappingContextEndpoint, ProjectEndpoint}
  ...
  "The service" should {
    ...
    "create a mapping context within project" in {
      ...
      Post(
        ...
        HttpEntity(ContentTypes.`text/plain(UTF-8)`, mappingContext1)
      ) ~> route ~> check {
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.mappingContextRepositoryFolderPath, projectId, mappingContext1)
          .toFile should exist
      }
      // create the second mapping context
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${MappingContextEndpoint.SEGMENT_CONTEXTS}",
        HttpEntity(ContentTypes.`text/plain(UTF-8)`, mappingContext2)
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        // validate that mapping context is updated in projects.json file
        val projects: JArray = TestUtil.getProjectJsonFile(ignifyrEngineConfig)
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.mappingContextRepositoryFolderPath, projectId, mappingContext2)
          .toFile should exist
      }
    }
    ...
    "delete a mapping context within a project" in {
      Delete(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${MappingContextEndpoint.SEGMENT_CONTEXTS}/$mappingContext2"
      ) ~> route ~> check {
    ...
    "upload a csv content to a mapping context" in {
      ...
      val file: File =
        FileOperations.getFileIfExists(getClass.getResource("/test-mapping-context/sample-unit-conversion.csv").getPath)
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${MappingContextEndpoint.SEGMENT_CONTEXTS}/$mappingContext1/${MappingContextEndpoint.SEGMENT_FILE}",
        formData.toEntity()
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
