...
class SchemaEndpointTest extends BaseEndpointTest with OnFhirTestContainer {
  ...
  val inferTaskWithSqlQuery: InferTask = InferTask(
    name = "test",
    mappingJobSourceSettings = Map(
      "source" ->
        SqlSourceSettings(
  ...
  val inferTaskWithPreprocessSql: InferTask = InferTask(
    name = "test",
    mappingJobSourceSettings = Map(
      "source" ->
        SqlSourceSettings(
  ...
  val schema1: SchemaDefinition = SchemaDefinition(
  ...
  val schema2: SchemaDefinition = SchemaDefinition(
    url = "https://example.com/fhir/StructureDefinition/schema2",
    version = SchemaDefinition.VERSION_LATEST,
    `type` = "Ty2",
    name = "name2",
  ...
  val schema2_2: SchemaDefinition = SchemaDefinition(
    url = "https://example.com/fhir/StructureDefinition/schema2",
    version = "0.9.12",
    `type` = "Ty2",
    name = "name2",
    description = Some("old-version-description2"),
    rootDefinition = None,
    fieldDefinitions = None
  )
  ...
  val schema3: SchemaDefinition = SchemaDefinition(
    url = "https://example.com/fhir/StructureDefinition/schema3",
    version = SchemaDefinition.VERSION_LATEST,
    `type` = "Ty3",
    name = "name3",
    ...
    fieldDefinitions = Some(
      Seq(
        SimpleStructureDefinition(
          id = "element-with-definition",
          path = "Ty3.element-with-definition",
          dataTypes = Some(
            Seq(
              DataTypeWithProfiles(
                dataType = FHIR_DATA_TYPES.CANONICAL,
                profiles = Some(Seq("http://hl7.org/fhir/StructureDefinition/canonical"))
              )
            )
          ),
          isPrimitive = true,
          ...
          patternValue = None,
          referringTo = None,
          short = Some("element-with-definition"),
          definition = Some("element definition"),
          comment = None,
          elements = None
        ),
        SimpleStructureDefinition(
          id = "element-with-no-definition",
          path = "Ty3.element-with-no-definition",
          dataTypes = Some(
            Seq(
              DataTypeWithProfiles(
                dataType = FHIR_DATA_TYPES.CANONICAL,
          ...
          patternValue = None,
          referringTo = None,
          short = Some("element-with-no-definition"),
          definition = None,
          comment = None,
          elements = None
        )
  ...
  val schema4: SchemaDefinition = SchemaDefinition(
    url = "https://example.com/fhir/StructureDefinition/schema4",
    version = SchemaDefinition.VERSION_LATEST,
    `type` = "Ty4",
    name = "name4",
    ...
    fieldDefinitions = Some(
      Seq(
        SimpleStructureDefinition(
          id = "element-with-short",
          path = "Ty4.element-with-short",
          dataTypes = Some(
            Seq(
              DataTypeWithProfiles(
                dataType = FHIR_DATA_TYPES.CANONICAL,
          ...
          patternValue = None,
          referringTo = None,
          short = Some("element-with-short"),
          definition = None,
          comment = None,
          elements = None
        ),
        SimpleStructureDefinition(
          id = "element-with-no-short",
          path = "Ty4.element-with-no-short",
          dataTypes = Some(
            Seq(
              DataTypeWithProfiles(
                dataType = FHIR_DATA_TYPES.CANONICAL,
  ...
  val schema5: SchemaDefinition = SchemaDefinition(
    id = "schema5",
    url = "https://example.com/fhir/StructureDefinition/schema2",
    version = SchemaDefinition.VERSION_LATEST,
    `type` = "Ty5",
  ...
  val mapping: FhirMapping = FhirMapping(
    id = "mapping",
    url = "http://example.com/mapping",
    name = "mapping",
    source = Seq(FhirMappingSource(alias = "test", url = "https://example.com/fhir/StructureDefinition/schema2")),
    context = Map.empty,
    mapping = Seq.empty
  )
  ...
  "The service" should {
    ...
    "create a schema within project" in {
      ...
      Post(
        ...
        HttpEntity(ContentTypes.`application/json`, writePretty(schema1))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        // validate that schema metadata file is updated
        val projects: JArray = TestUtil.getProjectJsonFile(ignifyrEngineConfig)
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.schemaRepositoryFolderPath, projectId, s"${schema1.id}${FileExtensions.JSON}")
          .toFile should exist
      }
      // create the second schema
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema2))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
        // validate that schema metadata file is updated
        val projects: JArray = TestUtil.getProjectJsonFile(ignifyrEngineConfig)
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.schemaRepositoryFolderPath, projectId, s"${schema2.id}${FileExtensions.JSON}")
          .toFile should exist
      }
    }
    ...
    "get a schema in a project" in {
      // get a schema
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${schema1.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      ...
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/123123"
      ) ~> route ~> check {
    ...
    "get a schema by url in a project" in {
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=${schema1.url}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "update a schema in a project" in {
      ...
      Put(
        ...
        HttpEntity(
          ...
          writePretty(schema1.copy(url = "https://example.com/fhir/StructureDefinition/schema3", name = schema3.name))
        )
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        ...
        (
          (projects.arr.find(p => (p \ "id").extract[String] == projectId).get \ "schemas")
            .asInstanceOf[JArray]
            .arr
            .find(m => (m \ "id").extract[String].contentEquals(schema1.id))
            .get \ "url"
        )
    ...
    "cannot delete a schema from a project if it is referenced by some mappings" in {
      ...
      Post(
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.mappingRepositoryFolderPath, projectId, s"${mapping.id}${FileExtensions.JSON}")
          .toFile should exist
      }

      // delete schema2
      Delete(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${schema2.id}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
      }
    }
    ...
    "create a schema having some elements with/without description" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema3))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
      }
      // retrieve the third schema
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=${schema3.url}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "create a schema having some elements with/without short" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema4))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
      }
      // retrieve the fourth schema
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=${schema4.url}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "infer the schema on a given table name and retrieve column types" in {
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_INFER}",
        HttpEntity(ContentTypes.`application/json`, writePretty(inferTaskWithTableName))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
        val fieldDefinitions = schema.fieldDefinitions.get
    ...
    "infer the schema on a given SQL query and retrieve column types" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_INFER}",
        HttpEntity(ContentTypes.`application/json`, writePretty(inferTaskWithSqlQuery))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
        val fieldDefinitions = schema.fieldDefinitions.get
    ...
    "infer the schema on a given SQL query with a preprocess SQL and retrieve column types" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_INFER}",
        HttpEntity(ContentTypes.`application/json`, writePretty(inferTaskWithPreprocessSql))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
    ...
    "create schemas having the same URL but different versions" in {
      Post(
        ...
        HttpEntity(
          ContentTypes.`application/json`,
          writePretty(schema5.copy(version = "some-version-other-than-latest"))
        )
      ) ~> route ~> check {
        ...
        FileUtils
          .getPath(ignifyrEngineConfig.schemaRepositoryFolderPath, projectId, s"${schema5.id}${FileExtensions.JSON}")
          .toFile should exist
      }
    }
    ...
    "cannot create a schema having the same id, url or name as another schema" in {
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema5))
      ) ~> route ~> check {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema5.copy(id = schema2.id)))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Conflict
      }
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema5.copy(name = schema2.name)))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Conflict
      }
    }
    ...
    "create an HTTP response with bad request for file data sources with wrong file extension" in {
      val erroneousInferTask = inferTaskWithSqlQuery.copy(
        mappingJobSourceSettings = inferTaskWithSqlQuery.mappingJobSourceSettings.updated(
          "source",
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_INFER}",
        HttpEntity(ContentTypes.`application/json`, writePretty(erroneousInferTask))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
        val response = responseAs[String]
        response should include("Type: https://ignifyr.io/errors/BadRequest")
      }
    }
    ...
    "create an HTTP response with bad request for wrong user credentials to access the DB" in {
      val erroneousInferTask = inferTaskWithSqlQuery.copy(mappingJobSourceSettings =
        inferTaskWithSqlQuery.mappingJobSourceSettings.updated(
          "source",
          SqlSourceSettings(
            name = "test-db-source",
            sourceUri = "https://aiccelerate.eu/data-integration-suite/test-data",
    ...
    "get structure definition of a schema to export" in {
      ...
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${schema3.id}?format=${SchemaFormats.STRUCTURE_DEFINITION}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
      ...
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/123123?format=${SchemaFormats.STRUCTURE_DEFINITION}"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.NotFound
      }
    }
    ...
    "import schema as structure definition" in {
      ...
      val bloodPressureSchemaUrl: String =
        "https://aiccelerate.eu/fhir/StructureDefinition/Ext-plt1-hsjd-blood-pressure"
      val bloodPressureSchemaName: String = "blood-pressure-schema"

      // read the StructureDefinition of the schema from file
      val schemaResource: Some[Resource] = Some(
        FileOperations.readJsonContentAsObject[Resource](
          FileOperations.getFileIfExists(getClass.getResource("/test-schemas/blood-pressure-schema.json").getPath)
        )
      )

      // create a schema by using StructureDefinition
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?format=${SchemaFormats.STRUCTURE_DEFINITION}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schemaResource))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      }
      ...
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=$bloodPressureSchemaUrl"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "import a non-existent StructureDefinition from a FHIR server" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_IMPORT}",
        HttpEntity(ContentTypes.`application/json`, writePretty(importSchemaSettings))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.BadRequest
        val response = responseAs[String]
    ...
    "import StructureDefinition from a FHIR server" in {
      ...
      // validate if the schema is imported correctly
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=$schemaURL"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "import StructureDefinitions from a ZIP of FHIR Profiles" in {
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}/${SchemaDefinitionEndpoint.SEGMENT_IMPORT_ZIP}",
        formData
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
      }
      ...
      // validate if the schema is imported correctly
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=$conditionSchemaURL"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
      ...
      // validate if the schema is imported correctly
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?url=$observationSchemaURL"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
        // validate the retrieved schema
        val schema: SchemaDefinition = JsonMethods.parse(responseAs[String]).extract[SchemaDefinition]
    ...
    "return an error when importing a FHIR profiles ZIP with missing referenced schemas" in {
      // create the FormData including the ZIP file
      val zipFilePath =
        Paths.get(getClass.getResource("/fhir-resources/fhir-profiles-missing-referenced-schemas.zip").toURI)
      val zipFileBytes = Files.readAllBytes(zipFilePath)
      val formData = Multipart.FormData(
        Multipart.FormData.BodyPart.Strict(
          "file",
      ...
      Post(
        ...
        response should include(
          "Detail: The schema with URL 'https://aiccelerate.eu/fhir/StructureDefinition/AIC-Condition' references a non-existent schema: 'https://aiccelerate.eu/fhir/StructureDefinition/AIC-Patient'. Ensure all referenced schemas exist."
        )
      }
    }
    ...
    "retrieve simplified element definitions of a schema" in {
      ...
      val schema: SchemaDefinition = SchemaDefinition(
        ...
        fieldDefinitions = Some(
          Seq(
            SimpleStructureDefinition(
              id = "element-with-definition",
              path = "Ty.element-with-definition",
              dataTypes = Some(
                Seq(
                  DataTypeWithProfiles(
                    dataType = FHIR_DATA_TYPES.CANONICAL,
            ...
            SimpleStructureDefinition(
              id = "element-with-no-definition",
              path = "Ty.element-with-no-definition",
              dataTypes = Some(
                Seq(
                  DataTypeWithProfiles(
                    dataType = FHIR_DATA_TYPES.CANONICAL,
      ...
      Post(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}",
        HttpEntity(ContentTypes.`application/json`, writePretty(schema))
      ) ~> route ~> check {
        status shouldEqual StatusCodes.Created
      }
      // retrieve the simplified element definitions of this schema
      Get(
        s"/${webServerConfig.baseUri}/${ProjectEndpoint.SEGMENT_PROJECTS}/$projectId/${SchemaDefinitionEndpoint.SEGMENT_SCHEMAS}?$QUERY_PARAM_TYPE=${SchemaFormats.SIMPLE_STRUCTURE_DEFINITION}&$QUERY_PARAM_URL=$schemaUrl"
      ) ~> route ~> check {
        status shouldEqual StatusCodes.OK
  ...
  override def beforeAll(): Unit = {
    ...
    val testPatientProfile: Resource = JsonMethods
      .parse(
        Source.fromInputStream(getClass.getResourceAsStream("/fhir-resources/custom-patient-profile.json")).mkString
      )
      .extract[Resource]
    onFhirClient
      .batch()
