package io.ignifyr.server.common.config
...
class WebServerConfig(webserverConfig: Config) {
  ...
  lazy val serverSsl: Boolean =
    Try(webserverConfig.getConfig("ssl").isEmpty || webserverConfig.getString("ssl.keystore") != null).getOrElse(false)
  ...
  lazy val baseUri: String = Try(webserverConfig.getString("base-uri")).getOrElse("ignifyr")
}
