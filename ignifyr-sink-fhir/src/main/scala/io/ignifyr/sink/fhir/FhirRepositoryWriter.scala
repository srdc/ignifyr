...
class FhirRepositoryWriter(sinkSettings: FhirRepositorySinkSettings) extends BaseSinkWriter(sinkSettings) {
  ...
  override def write(
    ...
    df
      .foreachPartition { partition: Iterator[FhirMappingResult] =>
        ...
        partition
          .grouped(IgnifyrConfig.engineConfig.fhirWriterBatchGroupSize)
          .foreach(rowGroup => {
  ...
  override def validate(): Unit = {
    ...
    try {
        ...
        onFhirClient
          ...
          .map(response => {
            if (!response.httpStatus.isSuccess()) {
              throw InvalidFhirRepositoryUrlException(
                s"Failed to retrieve capability statement for the FHIR Server at '${onFhirClient.getBaseUrl()}' which returns ${response.httpStatus}. Please make sure that the URL is correct."
              )
            }
          }),
      ...
      case te: TimeoutException =>
        throw InvalidFhirRepositoryUrlException(
          s"Failed to retrieve capability statement for the FHIR Server at '${onFhirClient.getBaseUrl()}' in 10 seconds. Please make sure that the URL is correct.",
          te
        )
      case e: Throwable =>
        throw InvalidFhirRepositoryUrlException(
          s"Failed to retrieve capability statement for the FHIR Server at '${onFhirClient.getBaseUrl()}'. Please make sure that the URL is correct.",
          e
        )
    }
  }
   ...
   * @return
   */
  private def prepareBatchRequest(
    ...
    mappingResults
      .foreach { mappingResult =>
        ...
        mappingResult.mappedFhirResource.get.fhirInteraction
    ...
    try {
      ...
      case fce: FhirClientException =>
        if (fce.serverResponse.isDefined && fce.serverResponse.get.outcomeIssues.nonEmpty) {
              ...
              element.copy(
                error = Some(
                  FhirMappingError(
                    ...
                    description =
                      "Resource is not a valid FHIR resource or does not conform to the indicated profiles. Additionally, this resource might be skipped due to duplicate entries in the batch/transaction!",
          ...
          val msg =
            s"FHIR repository at url ${sinkSettings.fhirRepoUrl} returned an unidentified error while writing the resources!"
          mappingResults
            .map(mr =>
              mr.copy(error =
                Some(
                  FhirMappingError( // Set the error
      ...
      case tout: TimeoutException =>
        val msg =
          s"FHIR repository at url ${sinkSettings.fhirRepoUrl} timeout for batch interaction while writing the resources!"
        mappingResults
          .map(mr =>
            mr.copy(error =
              Some(
    ...
    outcomeIssues
      .groupBy { issue =>
        if (issue.expression.isEmpty) {
          ...
          logger.error(
            s"Firely did not return an expression indicating the location of the OutcomeIssue: ${Serialization.write(issue)}"
          )
          None
        } else {
          if (issue.expression.size > 1) {
            logger.warn(
              s"There are more than one expression describing the location of the OutcomeIssue. I will continue with the first expression: ${issue.expression.head}"
            )
          }
          ...
          if (resourceEntryIndex.isEmpty) {
            logger.error(
              s"Entry index cannot be extracted from the expression given in the OutcomeIssue: ${Serialization.write(issue)}"
            )
          }
          resourceEntryIndex
        }
      }
      .collect { case (Some(index), issues) =>
        index -> issues
      }
  }
  ...
  private def checkResults(
    ...
    if (hasNonTransientErrors(mappingResults, responseBundle) || retry == 4) {
      ...
      responseBundle.responses.zipWithIndex // Add indexes to each response
   ...
   */
  private def hasNonTransientErrors(
      mappingResults: Seq[FhirMappingResult],
      responseBundle: FHIRTransactionBatchBundle
  ): Boolean = {
    responseBundle.responses.zipWithIndex
      .exists { case (r, i) =>
        ...
        !(
          r._2.httpStatus.intValue() == HttpStatus.SC_NOT_FOUND &&
            mappingResults(i).mappedFhirResource.get.fhirInteraction.exists(fint =>
              fint.`type` == FHIR_INTERACTIONS.PATCH && fint.condition.nonEmpty
            )
        )
      }
  }
   ...
   * @param ec
   */
  private def retryRequestsWithTransientError(
      mappingResults: Seq[FhirMappingResult],
      responseBundle: FHIRTransactionBatchBundle,
      problemsAccumulator: CollectionAccumulator[FhirMappingResult],
      onFhirClient: OnFhirNetworkClient,
    ...
    val transientIndices: Seq[Int] = responseBundle.responses.zipWithIndex.collect {
    ...
    logger.warn(
      s"Some mapped FHIR content (${mappingResultsNotPersisted.size} entries) is not persisted due to conflicts (409 conflict), retrying (retry #$retry) for them..."
    )
    Thread.sleep(50)
