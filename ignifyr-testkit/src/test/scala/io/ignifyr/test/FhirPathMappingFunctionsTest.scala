...
import io.ignifyr.engine.model.{FhirMapping, FhirMappingContextDefinition}
...
class FhirPathMappingFunctionsTest extends AsyncFlatSpec with IgnifyrTestSpec {

  // a simple json on which we can run mpp:nonEmptyLoopedFields function
  val loopJson: JValue =
    JsonMethods.parse(Source.fromInputStream(getClass.getResourceAsStream("/test-data/loop.json")).mkString)
  ...
  val unitConversionContextDefinition: FhirMappingContextDefinition =
    labResultMapping.context("labResultUnitConversion")
  ...
  "A FhirPathMappingsFunctions" should "correctly execute the getConcept function" in {
    mappingContextLoader.retrieveContext(conceptMapContextDefinition) map { mappingContext =>
      ...
      val sourceSystem = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, '1299-7', 'source_system')", JNull)
      sourceSystem shouldBe Some("http://loinc.org")
      val sourceDisplay = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, '1299-7', 'source_display')", JNull)
      sourceDisplay shouldBe Some("Trom (cumulative) given")
      val unit = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, '1299-7', 'unit')", JNull)
      unit shouldBe Some("mL")
      val profile = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, '1299-7', 'profile')", JNull)
      profile shouldBe Some("https://aiccelerate.eu/fhir/StructureDefinition/AIC-IntraOperativeObservation")

      val unknownCode = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, 'UNKNOWN_CODE', 'profile')", JNull)
      unknownCode shouldBe None
      val unknownColumn = FhirPathEvaluator()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString("mpp:getConcept(%obsConceptMap, '1299-7', 'UNKNOWN_COLUMN')", JNull)
  ...
  it should "correctly execute the convertAndReturnQuantity function" in {
    mappingContextLoader.retrieveContext(unitConversionContextDefinition) map { mappingContext =>
      ...
      val valueQuantity = FhirPathEvaluator()
      ...
      val unknownConversion = FhirPathEvaluator()
        .withDefaultFunctionLibraries()
        .withFunctionLibrary("mpp", fhirPathMappingFunctions)
        .evaluateOptionalString(
          "mpp:convertAndReturnQuantity(%labResultUnitConversion, 'UNKNOWN_CODE', 100, 'UNKNOWN_UNIT')",
          JNull
        )
