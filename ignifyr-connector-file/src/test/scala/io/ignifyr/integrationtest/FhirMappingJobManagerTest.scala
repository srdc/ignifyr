...
import io.ignifyr.engine.util.{FhirMappingJobFormatter, FhirMappingUtility, FileUtils}
...
class FhirMappingJobManagerTest
  ...
  private def deleteResources(): Assertion = {
    ...
    (1 to 10).foreach(i => {
      ...
      val f = onFhirClient.search("Observation").where("subject", patientReference).executeAndReturnBundle() map {
        observationBundle =>
          ...
          onFhirClient
            ...
            .where("subject", patientReference)
            .executeAndReturnBundle() map { medicationAdminBundle =>
  ...
  it should "execute the patient mapping task and return the results" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution = FhirMappingJobExecution(job = fhirMappingJob, mappingTasks = Seq(patientMappingTask)),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { mappingResults =>
  ...
  it should "not execute the draft patient mapping task and return error" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution = FhirMappingJobExecution(
        ...
        mappingTasks = Seq(patientMappingTaskWithDraftMapping)
      ),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) flatMap { _ =>
  ...
  it should "execute a mapping job with two data sources" in {
    val mappingJob = FhirMappingJobFormatter.readMappingJobFromFile(
      getClass.getResource("/patient-mapping-job-with-two-sources.json").toURI.getPath
    )

    val fhirMappingJobManager =
      new FhirMappingJobManager(mappingRepository, contextLoader, schemaRepository, Map.empty, sparkSession)
    fhirMappingJobManager.executeMappingJob(
      mappingJobExecution = FhirMappingJobExecution(mappingTasks = mappingJob.mappings, job = mappingJob),
      sourceSettings = mappingJob.sourceSettings,
      sinkSettings =
        mappingJob.sinkSettings.asInstanceOf[FhirRepositorySinkSettings].copy(fhirRepoUrl = onFhirClient.getBaseUrl())
    ) flatMap { _ =>
  ...
  it should "execute the mappings with FHIR Path patch" in {
    val patientExtraMappingWithPatch: FhirMappingTask = FhirMappingTask(
      ...
      sourceBinding =
        Map("source" -> FileSystemSource(path = "patients-extra.csv", contentType = SourceContentTypes.CSV))
    )
    val fhirMappingJobManager =
      new FhirMappingJobManager(mappingRepository, contextLoader, schemaRepository, Map.empty, sparkSession)
    fhirMappingJobManager
      ...
      .flatMap(_ =>
        fhirMappingJobManager.executeMappingJob(
          mappingJobExecution =
            FhirMappingJobExecution(mappingTasks = Seq(patientExtraMappingWithPatch), job = fhirMappingJob),
          sourceSettings = fhirMappingJob.sourceSettings,
          sinkSettings = fhirMappingJob.sinkSettings
        ) flatMap { response =>
          onFhirClient
            .read("Patient", FhirMappingUtility.getHashedId("Patient", "p1"))
            .executeAndReturnResource() flatMap { p1Resource =>
            onFhirClient
              .read("Patient", FhirMappingUtility.getHashedId("Patient", "p2"))
              .executeAndReturnResource() flatMap { p2Resource =>
  ...
  it should "execute the mappings with conditional FHIR Path patch" in {
    ...
    fhirMappingJobManager
      ...
      .flatMap(_ =>
        fhirMappingJobManager.executeMappingJob(
          mappingJobExecution =
            FhirMappingJobExecution(mappingTasks = Seq(patientExtraMappingWithConditionalPatch), job = fhirMappingJob),
          sourceSettings = fhirMappingJob.sourceSettings,
          sinkSettings = fhirMappingJob.sinkSettings
        ) flatMap { response =>
          response shouldBe ()
          onFhirClient
            ...
            onFhirClient
              ...
              fhirMappingJobManager.executeMappingJob(
                mappingJobExecution = FhirMappingJobExecution(
                  mappingTasks = Seq(patientExtraMappingWithConditionalPatch),
                  job = fhirMappingJob
                ),
                sourceSettings = fhirMappingJob.sourceSettings,
                sinkSettings = fhirMappingJob.sinkSettings
                ...
                onFhirClient
                  ...
                  onFhirClient
                    .read("Patient", FhirMappingUtility.getHashedId("Patient", "p2"))
                    .executeAndReturnResource() flatMap { p2Resource =>
                    (p1Resource \ "communication" \ "language" \ "coding" \ "code")
                      .extract[Seq[String]] shouldBe Seq("tr")
                    (p2Resource \ "communication" \ "language" \ "coding" \ "code")
                      .extract[Seq[String]] shouldBe Seq("en")
                  }
                }
              }
            }
  ...
  it should "execute the mapping with JSON patch" in {
    ...
    fhirMappingJobManager
      ...
      .flatMap(_ =>
        fhirMappingJobManager.executeMappingJob(
          mappingJobExecution = FhirMappingJobExecution(mappingTasks = Seq(jsonPatchMapping), job = fhirMappingJob),
          sourceSettings = fhirMappingJob.sourceSettings,
          sinkSettings = fhirMappingJob.sinkSettings
        ) flatMap { response =>
          response shouldBe ()
  ...
  it should "execute the patient mapping task with given tsv file and return the results" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution =
        FhirMappingJobExecution(job = fhirMappingJob, mappingTasks = Seq(patientTsvFileMappingTask)),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedFhirResource.get.mappedResource should not be None
  ...
  it should "execute the patient mapping task with given zip file and return the results" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution =
        FhirMappingJobExecution(job = fhirMappingJob, mappingTasks = Seq(patientZipFileMappingTask)),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedFhirResource.get.mappedResource should not be None
  ...
  it should "execute the other observation mapping task and return the results" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution =
        FhirMappingJobExecution(mappingTasks = Seq(otherObservationMappingTask), job = fhirMappingJob),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedFhirResource.get.mappedResource should not be None
      ...
      (observation1 \ "subject" \ "reference")
        .extract[String] shouldBe FhirMappingUtility.getHashedReference("Patient", "p1")
      ...
      (observation5 \ "meta" \ "profile")
        .extract[Seq[String]]
        .head shouldBe "https://aiccelerate.eu/fhir/StructureDefinition/AIC-IntraOperativeObservation"
      ...
      (results(8) \ "meta" \ "profile")
        .extract[Seq[String]]
        .head shouldBe "https://aiccelerate.eu/fhir/StructureDefinition/AIC-PEWSScore"
      (results(10) \ "meta" \ "profile")
        .extract[Seq[String]]
        .head shouldBe "https://aiccelerate.eu/fhir/StructureDefinition/AIC-MedicationAdministration"
  ...
  it should "execute the mapping job with multiple mapping tasks and write the results into a FHIR repository" in {
    ...
    fhirMappingJobManager.executeMappingJob(
      mappingJobExecution = FhirMappingJobExecution(
        mappingTasks = Seq(patientMappingTask, otherObservationMappingTask),
        job = fhirMappingJob
      ),
      sourceSettings = fhirMappingJob.sourceSettings,
      sinkSettings = fhirMappingJob.sinkSettings
      ...
      onFhirClient.read("Patient", FhirMappingUtility.getHashedId("Patient", "p8")).executeAndReturnResource() flatMap {
        p1Resource =>
          ...
          onFhirClient.search("Observation").where("code", "1035-5").executeAndReturnBundle() flatMap {
            observationBundle =>
              ...
              onFhirClient.search("MedicationAdministration").where("code", "313002").executeAndReturnBundle() flatMap {
                medicationAdministrationBundle =>
  ...
  it should "continue execute the mapping job when encounter without an error" in {
    val patientMappingTaskWithError: FhirMappingTask = FhirMappingTask(
      ...
      mappingRef = "https://aiccelerate.eu/fhir/mappings/patient-mapping",
      sourceBinding =
        Map("source" -> FileSystemSource(path = "patients-erroneous.csv", contentType = SourceContentTypes.CSV))
    )
    val fhirMappingJobManager = new FhirMappingJobManager(
      mappingRepository,
      contextLoader,
    ...
    val future = fhirMappingJobManager.executeMappingJob(
      mappingJobExecution = FhirMappingJobExecution(
  ...
  it should "execute the FhirMappingJob restored from a file" in {
    val lMappingJob =
      FhirMappingJobFormatter.readMappingJobFromFile(getClass.getResource("/test-mappingjob.json").toURI.getPath)

    val fhirMappingJobManager =
      new FhirMappingJobManager(mappingRepository, new MappingContextLoader, schemaRepository, Map.empty, sparkSession)
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution =
        FhirMappingJobExecution(mappingTasks = Seq(lMappingJob.mappings.head), job = fhirMappingJob),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { results =>
      results.size shouldBe 10
    }
  ...
  it should "execute the FhirMapping task with a terminology service" in {
    val terminologyServiceFolderPath =
      testResourceFolderPath("/terminology-service")
    ...
    val mappingTask = FhirMappingTask(
      "specimen-mapping-using-ts",
      "https://aiccelerate.eu/fhir/mappings/specimen-mapping-using-ts",
      Map("source" -> FileSystemSource("specimen.csv", SourceContentTypes.CSV))
    )

    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution = FhirMappingJobExecution(job = fhirMappingJob, mappingTasks = Seq(mappingTask)),
      fhirMappingJob.sourceSettings,
      Some(terminologyServiceSettings)
    ) flatMap { result =>
      ...
      FHIRUtil
        .extractValueOptionByPath[Seq[String]](resources.head, "type.coding.code")
        .getOrElse(Nil)
        .toSet shouldEqual Set("309068002", "111")
      FHIRUtil
        .extractValueOptionByPath[Seq[String]](resources.head, "type.coding.display")
        .getOrElse(Nil)
        .toSet shouldEqual Set("Specimen from skin", "Eiterprobe")
      FHIRUtil
        .extractValueOptionByPath[Seq[String]](resources.last, "type.coding.code")
        .getOrElse(Nil)
        .toSet shouldEqual Set("111")
    }
  }

  it should "execute the FhirMappingJob using an identity service" in {
    val testMappingJobWithIdentityServiceFilePath: String =
      getClass.getResource("/test-mappingjob-using-services.json").toURI.getPath
    ...
    fhirMappingJobManager
      .executeMappingJob(
        ...
        identityServiceSettings = lMappingJob
          .copy(sinkSettings =
            lMappingJob.sinkSettings
              .asInstanceOf[FhirRepositorySinkSettings]
              .copy(fhirRepoUrl = onFhirClient.getBaseUrl())
          )
          .getIdentityServiceSettings()
      ) map { res =>
      res shouldBe a[Unit]
    }
  }
  ...
  it should "execute the FhirMappingJob with preprocess" in {
    ...
    fhirMappingJobManager.executeMappingTaskAndReturn(
      mappingJobExecution =
        FhirMappingJobExecution(mappingTasks = Seq(patientMappingTaskWithPreprocess), job = fhirMappingJob),
      mappingJobSourceSettings = fhirMappingJob.sourceSettings
    ) map { mappingResults =>
      val results = mappingResults.map(r => {
        r.mappedFhirResource.get.mappedResource should not be None
