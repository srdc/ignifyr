 ...
 */
class CustomMappingFunctions(context: FhirPathEnvironment, current: Seq[FhirPathResult])
  ...
  @FhirPathFunction(
    documentation = FhirPathFunctionDocumentation(
      detail =
        "Decodes the given data and converts it to an array of space separated numbers. Returns the space separated numbers concatenated in a string.",
      usageWarnings = None,
      parameters = Some(
        Seq(
          FhirPathFunctionParameter(
  ...
  def createTimeSeriesData(dataExpr: ExpressionContext): Seq[FhirPathResult] = {
    ...
    if (dataResult.length > 1 || !dataResult.head.isInstanceOf[FhirPathString]) {
      throw new FhirPathException(
        s"Invalid function call 'createTimeSeriesData', given expression for origin parameter: ${dataExpr.getText} should return a single, string value!"
      )
    }
    ...
    val decodedNumbers: Seq[String] = Range(0, encoded.length, 2).map(i => {
      val bf: ByteBuffer =
        ByteBuffer.wrap(util.Arrays.copyOfRange(encoded, i, i + 2)).order(java.nio.ByteOrder.LITTLE_ENDIAN)
