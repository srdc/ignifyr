  ...
  override def execute(args: Seq[String], context: CommandExecutionContext): CommandExecutionContext = {
    if (context.fhirMappingJob.isEmpty) {
      ...
      val fhirMappingJobManager = new FhirMappingJobManager(
        context.ignifyrEngine.mappingRepo,
        context.ignifyrEngine.contextLoader,
        context.ignifyrEngine.schemaLoader,
        context.ignifyrEngine.functionLibraries,
        context.ignifyrEngine.sparkSession
      )
      ...
      if (!isStreamingJob(mappingJob)) {
        ...
        if (args.isEmpty) {
          ...
          context.ignifyrEngine.runningJobRegistry.registerBatchJob(
            mappingJobExecution,
            Some(f),
        ...
        fhirMappingJobManager
          ...
          .foreach(sq =>
            context.ignifyrEngine.runningJobRegistry.registerStreamingQuery(mappingJobExecution, sq._1, sq._2)
          )
      }
    }
    context
