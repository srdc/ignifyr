    ...
    if (args.isEmpty || args.size < 2) {
      ...
      mappingTaskName match {
        case None =>
          ...
          context.ignifyrEngine.runningJobRegistry.stopJobExecution(jobId, executionId)
        case Some(name) =>
          // Stop a single mappingTask
          context.ignifyrEngine.runningJobRegistry.stopMappingExecution(jobId, executionId, name)
      }
