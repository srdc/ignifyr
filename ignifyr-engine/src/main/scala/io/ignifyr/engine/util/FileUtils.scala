package io.ignifyr.engine.util
...
object FileUtils {
   ...
   * it is appended to the root context path defined by [[IgnifyrConfig.engineConfig.contextPath]].
   * This method is used for file access within Ignifyr.
   *
  ...
  def getPath(path: String, paths: String*): Path = {
    ...
    val resultingPath =
      ...
      else Paths.get(IgnifyrConfig.engineConfig.contextPath, givenPath.toString)
