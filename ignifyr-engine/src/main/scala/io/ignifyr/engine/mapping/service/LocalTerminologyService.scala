...
import io.ignifyr.engine.mapping.service.LocalTerminologyService.{
  CodeSystemFileColumns,
  ConceptMapFileColumns,
  equivalenceCodes
}
import io.ignifyr.engine.model.exception.FhirMappingException
import io.ignifyr.engine.model.{ConceptMapFile, LocalFhirTerminologyServiceSettings}
 ...
 */
class LocalTerminologyService(settings: LocalFhirTerminologyServiceSettings)
  ...
  val relatedFiles: Map[String, File] =
    IOUtil
      .getFilesFromFolder(
        new File(FileUtils.getPath(settings.folderPath).toUri),
        recursively = true,
        ignoreHidden = true,
        withExtension = Some(FileUtils.FileExtensions.CSV.toString)
      )
      .map(f => f.getName -> f)
      .toMap
            ...
            relatedFiles.get(cmf.name) match {
              case None =>
                throw FhirMappingException(
                  s"Missing Ignifyr ConceptMap CSV file '${cmf.name}' within configured folder '${settings.folderPath}'!"
                )
  ...
  val codeSystems: Map[String, Map[String, (String, Map[String, String])]] =
    settings.codeSystemFiles
      .map(cs =>
        cs.codeSystem -> parseCodeSystemGivenInCsv(
          relatedFiles.get(cs.name) match {
            case None =>
              throw FhirMappingException(
                s"Missing ignifyr CodeSystem CSV file '${cs.name}' within configured folder '${settings.folderPath}'!"
              )
            case Some(f) => f.getAbsolutePath
          }
        )
  ...
  override def translate(
    ...
    Future.apply {
      if (version.isDefined || conceptMapVersion.isDefined || reverse)
        throw FhirMappingException(
          "Only simple translate statements are supported in mappings with a local terminology service!, 'version', 'conceptMapVersion', 'reverse' parameters are not supported!"
        )
      ...
      conceptMaps
        ...
        .map(conceptMap =>
          conceptMap
            ...
            .getOrElse(
              constructParametersResourceForNoTranslation(
                s"Source system and code ($system, $code) not found withing the concept map $conceptMapUrl!"
              )
            )
        )
  ...
  override def translate(code: String, system: String, conceptMapUrl: String): Future[JObject] =
    translate(code, system, conceptMapUrl, None, None, reverse = false)
  ...
  override def translate(
    ...
    Future.apply {
      if (conceptMapVersion.isDefined | reverse)
        throw FhirMappingException(
          "Only simple translate statements are supported in mappings with a local terminology service!, The 'conceptMapVersion' or 'reverse' parameters are not supported!"
        )
      ...
      conceptMaps
        ...
        .map(conceptMap =>
          getSystemCodesFromCodeableConceptOrCoding(codingOrCodeableConcept) match {
            case Nil =>
              constructParametersResourceForNoTranslation(
                s"Given FHIR Coding or CodeableConcept element does not include system or code fields!"
              )
            ...
            case multipleCodes =>
              multipleCodes
                ...
                case Nil =>
                  constructParametersResourceForNoTranslation(
                    s"Code and system pairs given in CodeableConcept are not found within the concept map $conceptMapUrl!"
                  )
  ...
  override def translate(codingOrCodeableConcept: JObject, conceptMapUrl: String): Future[JObject] =
    translate(codingOrCodeableConcept, conceptMapUrl, None, reverse = false)
  ...
  override def translate(
    ...
    Future.apply {
      if (version.isDefined || reverse)
        throw FhirMappingException(
          "Only simple translate statements are supported in mappings with a local terminology service!. The 'version' or 'reverse' parameters are not supported!"
        )

      if (source.isEmpty && target.isEmpty)
        throw FhirMappingException(
          "Only simple translate statements are supported in mappings with a local terminology service!, The 'source' or 'target' parameter should be given!"
        )
      ...
      conceptMaps
        ...
        case Nil =>
          constructParametersResourceForNoTranslation(
            s"Concept map from source value set ${source.getOrElse("?")} to target value set ${target.getOrElse("?")} is not found!"
          )
        case foundConceptMaps =>
          foundConceptMaps.flatMap(conceptMap => conceptMap.get(system -> code)).flatten match {
            case Nil =>
              constructParametersResourceForNoTranslation(
                s"Source system and code ($system, $code) not found within any concept map from source value set ${source.getOrElse("?")} to target value set ${target.getOrElse("?")}!"
              )
            case matches => constructParametersResourceForTranslate(matches)
          }
      }
  ...
  override def translate(
      ...
      source: Option[String],
      target: Option[String]
  ): Future[JObject] = translate(code, system, source, target, None, reverse = false)
  ...
  override def translate(
    ...
    Future.apply {
      if (reverse)
        throw FhirMappingException(
          "Only simple translate statements are supported in mappings with a local terminology service! The 'reverse' parameter is not supported!"
        )
          ...
          getSystemCodesFromCodeableConceptOrCoding(codingOrCodeableConcept) match {
            ...
            case Seq(system -> code) =>
              foundConceptMaps.flatMap(_.get(system -> code)).flatten match {
                case Nil =>
                  constructParametersResourceForNoTranslation(
                    s"Source system and code ($system, $code) not found withing the concept map from source value set ${source.getOrElse("?")} to target value set ${target.getOrElse("?")}!"
                  )
                case matches => constructParametersResourceForTranslate(matches)
              }
            case multipleCodes =>
              multipleCodes
                ...
                case Nil =>
                  constructParametersResourceForNoTranslation(
                    s"Code and system pairs given in CodeableConcept are not found within the concept map from source value set ${source.getOrElse("?")} to target value set ${target.getOrElse("?")}!"
                  )
                case matches => constructParametersResourceForTranslate(matches)
              }
          }
    ...
    codingOrCodeableConcept.obj
      ...
      case Some(JArray(codings)) =>
        ...
        codings
          ...
          .map(c =>
            FHIRUtil.extractValueOption[String](c, "system") ->
      ...
      case None =>
        val sourceCode = FHIRUtil.extractValueOption[String](codingOrCodeableConcept, "system") -> FHIRUtil
          .extractValueOption[String](codingOrCodeableConcept, "code")
        sourceCode match {
  ...
  override def translate(
      ...
      source: Option[String],
      target: Option[String]
  ): Future[JObject] = translate(codingOrCodeableConcept, source, target, reverse = false)
  ...
  override def lookup(
    ...
    Future.apply {
      if (version.isDefined || date.isDefined || properties.nonEmpty)
        throw FhirMappingException(
          "Only simple lookup operation is supported in mappings with a local terminology service! The 'version', 'date', or 'properties' parameter is not supported!"
        )

      codeSystems
        .get(system)
        .flatMap(_.get(code))
        .map { case (display, designations) =>
          constructParametersResourceForLookup(system, display, designations, displayLanguage)
        }
    }
  }
  ...
  override def lookup(code: String, system: String): Future[Option[JObject]] =
    lookup(code, system, None, None, None, Nil)
  ...
  override def lookup(
    ...
    Future.apply {
      if (properties.nonEmpty || date.isDefined)
        throw FhirMappingException(
          "Only simple lookup operation is supported in mappings with a local terminology service! The 'properties' or 'date' parameter is not supported!"
        )

      val (system, code) =
        FHIRUtil.extractValue[String](coding, "system") -> FHIRUtil.extractValue[String](coding, "code")
   ...
   * @return
   */
  private def constructParametersResourceForLookup(
      system: String,
      display: String,
    ...
    JObject(
      "parameter" -> JArray(
        List(
          ...
          JObject(
            "name" -> JString("display"),
            "valueString" -> JString(displayLanguage.flatMap(l => designations.get(l)).getOrElse(display))
          )
        )
      )
    )
   ...
   * @return
   */
  private def constructParametersResourceForTranslate(
      matchings: Seq[(String, String, Option[String], String)]
  ): JObject = {
    JObject(
      "parameter" -> JArray(
        JObject("name" -> JString("result"), "valueBoolean" -> JBool(true)) +:
          matchings.map { case (system, code, display, equivalence) =>
            JObject(
              "name" -> JString("match"),
              "part" -> JArray(
                List(
                  ...
                  JObject(
                    ...
                    "valueCoding" -> JObject(
                      List("system" -> JString(system), "code" -> JString(code)) ++ display.map(d =>
                        "display" -> JString(d)
                      )
                    )
                  )
   ...
   */
  private def parseConceptMapGivenInCsv(
      filePath: String
  ): Map[(String, String), Seq[(String, String, Option[String], String)]] = {
    try {
      CsvUtil
        ...
        .map(columns =>
          (columns(ConceptMapFileColumns.SOURCE_SYSTEM) -> columns(ConceptMapFileColumns.SOURCE_CODE)) ->
            (
              ...
              columns
                .get(ConceptMapFileColumns.EQUIVALENCE)
                .filter(e => equivalenceCodes.contains(e))
                .getOrElse("equivalent") // If there is no equivalence column, we assume all matching are equivalent
            )
        )
        .groupBy(_._1)
        .view
      ...
      case t: CsvReadException =>
        throw FhirMappingException(s"Invalid ignifyr concept map CSV file $filePath!", t)
      case t: Throwable =>
        throw FhirMappingException(
          s"Invalid ignifyr concept map CSV file $filePath! Columns ${Set(ConceptMapFileColumns.SOURCE_SYSTEM, ConceptMapFileColumns.SOURCE_CODE, ConceptMapFileColumns.TARGET_SYSTEM, ConceptMapFileColumns.TARGET_CODE).mkString(",")} are mandatory for concept map!",
          t
        )
    }
  }
  ...
  private def parseCodeSystemGivenInCsv(filePath: String): Map[String, (String, Map[String, String])] = {
    try {
      ...
      case t: Throwable =>
        throw FhirMappingException(
          s"Invalid ignifyr CodeSystem CSV file $filePath! Columns ${Set(CodeSystemFileColumns.CODE, CodeSystemFileColumns.DISPLAY).mkString(",")} are mandatory for code system definitions!",
          t
        )
    }
  }
