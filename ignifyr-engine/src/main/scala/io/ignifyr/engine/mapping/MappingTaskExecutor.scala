...
import io.ignifyr.common.util.ExceptionUtil
...
import io.ignifyr.engine.model.{FhirMappingError, FhirMappingErrorCodes, FhirMappingResult, MappedFhirResource}
...
object MappingTaskExecutor {
  ...
  private def executeMappingOnInput(
    ...
    val results =
      try {
        val mappedResources =
          Await.result(fhirMappingService.mapToFhir(jo, otherInputs), IgnifyrConfig.engineConfig.mappingTimeout)
            ...
            FhirMappingResult(
              ...
              error = Some(
                FhirMappingError(
                  ...
                  description =
                    s"A single row could not be mapped to FHIR in ${IgnifyrConfig.engineConfig.mappingTimeout.toString}!"
                )
              ),
              executionId = executionId,
              projectId = fhirMappingService.projectId
