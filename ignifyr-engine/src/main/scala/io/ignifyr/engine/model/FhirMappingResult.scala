 ...
 *
 * @param resourceType       Sink-routing discriminator lifted from the mapped output JSON's `resourceType`
 *                           field: the FHIR resource type for FHIR outputs, or a target-specific type/table
 *                           name for non-FHIR outputs (flat/tabular mappings). None when the output carries
 *                           no discriminator. Sinks may use it only for routing/partitioning (e.g. the file
 *                           sink's partition-by-resource-type layout) — it is never validated against FHIR.
