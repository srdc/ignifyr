...
import io.ignifyr.engine.util.{FileUtils, SparkUtil}
...
class FileStreamInputArchiverTest extends AnyFlatSpec with Matchers {
  ...
  "FileStreamInputArchiver" should "not apply archiving/deletion for a streaming job with archive mode is off" in {
    ...
    org.apache.commons.io.FileUtils.deleteDirectory(new File(IgnifyrConfig.sparkCheckpointDirectory))
  }
  ...
  "FileStreamInputArchiver" should "apply archiving for a streaming job" in {
    ...
    val relPath = FileUtils
      .getPath("")
      .toAbsolutePath
      .relativize(Paths.get(IgnifyrConfig.sparkCheckpointDirectory, "test.csv").toAbsolutePath)
    // The relative path is appended to the base archive folder so that the path of the original input file is preserved
    val finalArchivePath = Paths.get(IgnifyrConfig.engineConfig.archiveFolder, relPath.toString)
  ...
  private def initializeSparkFiles(jobId: String, mappingTaskName: String): File = {
    ...
    // Path of test.csv
    val testCsvFile = Paths.get(IgnifyrConfig.sparkCheckpointDirectory, "test.csv").toFile
    // Ensure the parent directories exist, if not, create them
    sourceFile.getParentFile.mkdirs()
    testCsvFile.getParentFile.mkdirs()
    ...
    Paths
      .get(
        SparkUtil.getSourceDirectoryPath(
          Paths.get(IgnifyrConfig.sparkCheckpointDirectory, jobId, mappingTaskName.hashCode.toString)
        ),
        fileName
      )
      .toFile
