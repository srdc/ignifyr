...
import io.ignifyr.engine.model.{CodeSystemFile, ConceptMapFile, LocalFhirTerminologyServiceSettings}
...
class LocalTerminologyServiceTest extends AsyncFlatSpec with IgnifyrTestSpec {
  ...
  val settings: LocalFhirTerminologyServiceSettings = LocalFhirTerminologyServiceSettings(
  ...
  "A LocalTerminologyService" should "translate code via concept url with single match" in {
    var result = Await.result(
      localTerminologyService.translate(
        "ACNE",
  ...
  it should "translate code via concept url with multiple match" in {
    ...
    FHIRUtil
      .getParameterValueByPath(result, "match.concept")
      .map(c => (c \ "code").extract[String])
      .toSet shouldEqual Set("119323008", "119323009")
  }
  ...
  it should "return no translation via concept url if no system is matching" in {
    val result = Await.result(
      localTerminologyService.translate("ACNFLD", "NO-MATCHING", "http://example.com/fhir/ConceptMap/sample1"),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    FHIRUtil.getParameterValueByName(result, "result").map(_.extract[Boolean]) shouldEqual Some(false)
  }
  ...
  it should "return no translation via concept url if no conceptUrl" in {
    val result = Await.result(
      localTerminologyService
        .translate("ACNFLD", "http://terminology.hl7.org/CodeSystem/v2-0487", "http://example.com/fhir/ConceptMap/NO"),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    FHIRUtil.getParameterValueByName(result, "result").map(_.extract[Boolean]) shouldEqual Some(false)
  }
  ...
  it should "translate code via source and target value set" in {
    var result = Await.result(
      localTerminologyService.translate(
        "ACNE",
        "http://terminology.hl7.org/CodeSystem/v2-0487",
        Some("http://terminology.hl7.org/ValueSet/v2-0487"),
        Some("http://snomed.info/sct?fhir_vs")
      ),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    FHIRUtil.getParameterValueByName(result, "result").map(_.extract[Boolean]) shouldEqual Some(true)
  ...
  it should "translate CodeableConcept via  concept url" in {
    ...
    val result = Await.result(
      localTerminologyService.translate(codeableConcept.parseJson, "http://example.com/fhir/ConceptMap/sample1"),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    FHIRUtil.getParameterValueByName(result, "result").map(_.extract[Boolean]) shouldEqual Some(true)
    FHIRUtil
  ...
  it should "translate Coding via  concept url" in {
    ...
    import io.onfhir.util.JsonFormatter._
    val result = Await.result(
      localTerminologyService.translate(coding.parseJson, "http://example.com/fhir/ConceptMap/sample1"),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    FHIRUtil.getParameterValueByName(result, "result").map(_.extract[Boolean]) shouldEqual Some(true)
    FHIRUtil
  ...
  it should "lookup a code that exist" in {
    val result = Await.result(
      localTerminologyService.lookup("119323008", "http://snomed.info/sct"),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
  ...
  it should "lookup a code with a language that exist" in {
    val result = Await.result(
      localTerminologyService.lookup("119323008", "http://snomed.info/sct", None, None, Some("de"), Nil),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    result.isEmpty shouldBe (false)
  ...
  it should "return empty for a code that does not exist" in {
    val result =
      Await.result(localTerminologyService.lookup("123", "http://snomed.info/sct"), FiniteDuration(5, TimeUnit.SECONDS))
    result.isEmpty shouldBe (true)
  }

  it should "lookup a code with a language where designation does not exist (returning default display)" in {
    val result = Await.result(
      localTerminologyService.lookup("119323008", "http://snomed.info/sct", None, None, Some("tr"), Nil),
      FiniteDuration(5, TimeUnit.SECONDS)
    )
    result.isEmpty shouldBe (false)
    FHIRUtil.getParameterValueByName(result.get, "display").map(_.extract[String]) shouldBe Some("Pus specimen")
