...
class SchedulingTest extends AnyFlatSpec with BeforeAndAfterAll with IgnifyrTestSpec with OnFhirTestContainer {
  ...
  val ignifyrDb: Path =
    Paths.get(resourcePath.resolve(IgnifyrConfig.engineConfig.ignifyrDbFolderPath).resolve("scheduler"))

  val mappingJobScheduler: MappingJobScheduler = MappingJobScheduler(scheduler, ignifyrDb.toUri)
  ...
  it should "schedule a FhirMappingJob with cron and sink settings restored from a file" in {
    ...
    val directory = new File(ignifyrDb.toUri)
